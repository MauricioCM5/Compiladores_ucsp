...
public:
    ...
    // Factor_more ::= Factor | identifier Type_Factor
    ...
    void parseVarDecl(){
        printf("VarDecl::= identifier VarDecl_more\n");
    ...
    void parseLiteral() {
        ...
        // Literal ::= integer_literal | boolean_literal | char_literal | string_literal
        if (value == "true" || value == "false") {advance(); return;}
        ...
        if (value.size() >= 3 && value.front() == '\'' && value.back() == '\'') {advance(); return;}
